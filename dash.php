...
<div class="row">
<div class="col-lg-6">
<span class="logo">QUI TECH</span></div>
<?php
...
//ranking start
if (@$_GET['q'] == 2) {
  // Seleccionar la base de datos 'project'
  mysqli_select_db($con, 'project') or die('No se pudo seleccionar la base de datos');
  
  // Procesar el formulario si se ha enviado un nuevo enlace de YouTube
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      foreach ($_POST['youtube_link'] as $eid => $link) {
          // Escapar y sanitizar el enlace para evitar SQL Injection
          $youtube_link = mysqli_real_escape_string($con, $link);
          
          // Actualizar el enlace de YouTube en la base de datos para el examen con el ID $eid
          mysqli_query($con, "UPDATE history SET youtube_link='$youtube_link' WHERE eid='$eid'") or die('Error: ' . mysqli_error($con));
      }
      echo '<div class="alert alert-success" role="alert">Los enlaces de YouTube han sido actualizados correctamente.</div>';
  }
  
  // Obtener la información del historial de exámenes del usuario junto con su información personal
  $q_history = mysqli_query($con, 
      "SELECT h.*, u.name, u.gender, u.college 
       FROM history h 
       JOIN user u ON h.email = u.email 
       ORDER BY h.date DESC") or die('Error197: ' . mysqli_error($con));
      ...
      <div class="panel title">
          <table class="table table-striped title1">
              <tr style="color:red">
                  ...
                  <td><b>Nombre</b></td>
                  <td><b>Género</b></td>
                  <td><b>Institución Académica</b></td>
                  <td><b>Examen</b></td>
                  <td><b>Preguntas Resueltas</b></td>
                  <td><b>Correctas</b></td>
                  <td><b>Incorrectas</b></td>
                  <td><b>Calificación</b></td>
                  <td><b>Promedio (%)</b></td>
                  <td><b>Enlace de YouTube</b></td>
                  <td><b>Subir</b></td> <!-- Subir -->
              </tr>';

  $c = 0;
  while ($row = mysqli_fetch_array($q_history)) {
      $eid = $row['eid'];
      $score = $row['score'];
      $wrong = $row['wrong'];
      $sahi = $row['sahi'];
      ...
      $q_quiz = mysqli_query($con, "SELECT title FROM quiz WHERE eid='$eid'") or die('Error208: ' . mysqli_error($con));
      $row_quiz = mysqli_fetch_array($q_quiz);
      $title = $row_quiz['title'];
      
      // Obtener el enlace de YouTube si existe
      $youtube_link = $row['youtube_link']; // Campo nuevo para almacenar el enlace de YouTube
      
      // Calcular el promedio como porcentaje
      if ($qa > 0) {
          $percentage = round(($sahi / $qa) * 100);
          if ($percentage < 0) {
              $percentage = 0;
          }
      } else {
      ...
      // Determinar la calificación basada en el porcentaje
      if ($percentage >= 90) {
          $grade = 'A';
      ...
      echo '<tr>
              ...
              <td>' . $grade . '</td>
              <td>' . $percentage . '%</td>
              <td><input type="text" name="youtube_link['.$eid.']" value="'.$youtube_link.'" style="width: 100%;"></td>
              <td><button type="submit" class="btn btn-primary" name="submit">Actualizar</button></td>
            </tr>';
